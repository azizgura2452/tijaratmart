	...
	public function info() {
		...
		if ($order_info) {
			...
			foreach ($products as $product) {
				...
				// Display prices
				$tax_class_id = $this->tax->getProductTax($product['product_id']);
				if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
					$unit_price = $this->tax->calculate($product['price'], $tax_class_id, $this->config->get('config_tax'));
					$unit_price_regular = $product['price'];
					$price = $this->currency->format($unit_price_regular, $this->session->data['currency']);
			...
			'sellerhref'       => $this->url->link('vendor/vendor/edit', 'token=' . $this->session->data['token'] . '&vendor_id=' . $ids, true),			
			/* xml */
			
					'quantity'		   => $product['quantity'],
					'price'    		   => $price, //$this->currency->format($product['price'] + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']),
					'total'    		   => $this->currency->format($product['total'] + ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value']),
					'tax_rate_name'    => $tax_rate_name, //custom tax
					'tax'       	   => $this->currency->format(($this->tax->getTax($product['price'], $tax_class_id)*$product['quantity']), $this->session->data['currency']), //custom tax
			...
			//Custom GST Module Order receipt
			$totals = calculateGst($totals);
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				if ($store_info) {
					...
					$store_zone_id = $store_info['config_zone_id']; //Custom GST Module
				} else {
				...
				//Custom GST Module (To apply CGST/SGST or IGST)
				$localOrder = true;
				if($store_zone_id != $order_info['shipping_zone_id']){
					$localOrder = false;
				}
				...
				foreach ($products as $product) {
					...
					$product_data[] = array(
						...
						'price'    => $price, //$this->currency->format($product['price'] + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']),
						'total'    => $this->currency->format($product['total'] + ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value']),
						'tax_amount' => $this->tax->getTax($product['price'], $tax_class_id)*$product['quantity'],
						'tax_rate_figure'	=>	$tax_rate_figure,
						'tax_rate_name' => $tax_rate_name, //custom tax
						'tax'       => $this->currency->format(($this->tax->getTax($product['price'], $tax_class_id)*$product['quantity']), $this->session->data['currency']), //custom tax
					);
				}
