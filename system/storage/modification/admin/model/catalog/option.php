...
class ModelCatalogOption extends Model {
	public function addOption($data) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "option` SET type = '" . $this->db->escape($data['type']) . "', sort_order = '" . (int)$data['sort_order'] . "',option_style = '" . $this->db->escape($data['option_style']) . "',background_color = '" . $this->db->escape($data['background_color']) . "',text_color = '" . $this->db->escape($data['text_color']) . "'");

		$option_id = $this->db->getLastId();

		foreach ($data['option_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "option_description SET option_id = '" . (int)$option_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', option_description = '" . $this->db->escape($value['option_description']) . "', option_name = '" . $this->db->escape($value['option_name']) . "', button_text = '" . $this->db->escape($value['button_text']) . "'");
		}
	...
	public function editOption($option_id, $data) {
		$this->db->query("UPDATE `" . DB_PREFIX . "option` SET type = '" . $this->db->escape($data['type']) . "', sort_order = '" . (int)$data['sort_order'] . "',option_style = '" . $this->db->escape($data['option_style']) . "',background_color = '" . $this->db->escape($data['background_color']) . "',text_color = '" . $this->db->escape($data['text_color']) . "' WHERE option_id = '" . (int)$option_id . "'");
	...
	public function getOptionDescriptions($option_id) {
		...
		foreach ($query->rows as $result) {
			$option_data[$result['language_id']] = array('name' => $result['name'],'option_description' => isset($result['option_description']) ? $result['option_description'] :'','option_name' => isset($result['option_name']) ? $result['option_name'] : '','button_text' => isset($result['button_text']) ? $result['button_text'] : '');
		}
