...
class ControllerJournal3Checkout extends \Journal3\Opencart\Controller {
	...
	public function index() {
		...
		if (!$this->journal3->isDev() && ((Arr::get($this->request->get, 'route') === 'journal3/checkout') || (Arr::get($this->request->get, '_route_') === 'journal3/checkout'))) {
		...
		if ($this->customer->isLogged() && $this->model_journal3_newsletter->isSubscribed($this->customer->getEmail())) {
			$newsletter = false;
		}
	...
	private function getCheckoutData($data) {
		...
		return array(
			...
			'login_email'           => '',
			'login_password'        => '',
			'guest'                 => (bool)$this->config->get('config_checkout_guest'),
