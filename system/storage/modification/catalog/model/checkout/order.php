...
$this->load->language('mail/order');
		...
		if ($order_info) {
			...
			if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				// Stock subtraction
				$order_product_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product  op LEFT JOIN " . DB_PREFIX . "vendor_order_product vop ON (op.order_id = vop.order_id) WHERE op.order_id = '" . (int)$order_id . "' GROUP BY vop.order_product_id");
			...
			/* xml */
			$data['text_byseller'] = $language->get('text_byseller');		
			/* xml */
				...
				if(!empty($orderss_query->rows)) {			
					...
					if(!empty($vendorinfo->row['email'])) {
						...
						$sellerproduct['store_name'] = $order_info['store_name'];
						$sellerproduct['text_order_id'] = $language->get('text_order_id');
						$sellerproduct['text_date_added'] = $language->get('text_date_added');
						$sellerproduct['text_order_status'] = $language->get('text_order_status');
						...
						if($customer2vendors==1){
						 $sellerproduct['customer2vendor'] = $customer2vendors;
						} else {
						 $sellerproduct['customer2vendor']= false;
						}
						...
						// Order Totals
							...
							$sellerproductorder_totals = $this->model_account_order->getOrderTotals($order_info['order_id']);
