...
class ControllerVendorProduct extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_vendor_product->addProduct($this->request->post);
			...
			$this->response->redirect($this->url->link('vendor/product', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('vendor/product', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('vendor/product/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['copy'] = $this->url->link('vendor/product/copy', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('vendor/product/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			...
			if (!$result['status'] || $result['status']==2) {
				$statuss = $this->url->link('vendor/product/status', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url, true);
			} else {
				$statuss = '';
			}
			...
			$data['products'][] = array(
				...
				/* 18 02 2020 */
				'sellerpage'       => $this->url->link('vendor/vendor/edit', 'token=' . $this->session->data['token'] . '&vendor_id=' . $result['vendor_id'] . $url, true),
				/* 18 02 2020 */
				'edit'       => $this->url->link('vendor/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url, true)
			);
		...
		$data['text_approvepending'] 	= $this->language->get('text_approvepending');
		/* 2020 */
		...
		$data['sort_vendor'] = $this->url->link('vendor/product', 'token=' . $this->session->data['token'] . '&sort=v2p.vendor' . $url, true);
		/* 18 02 2020 */
		$data['sort_storename'] = $this->url->link('vendor/product', 'token=' . $this->session->data['token'] . '&sort=vd.vendorstorename' . $url, true);
		/* 18 02 2020 */
		$data['sort_name'] = $this->url->link('vendor/product', 'token=' . $this->session->data['token'] . '&sort=pd.name' . $url, true);
		$data['sort_model'] = $this->url->link('vendor/product', 'token=' . $this->session->data['token'] . '&sort=p.model' . $url, true);
		$data['sort_price'] = $this->url->link('vendor/product', 'token=' . $this->session->data['token'] . '&sort=p.price' . $url, true);
		$data['sort_quantity'] = $this->url->link('vendor/product', 'token=' . $this->session->data['token'] . '&sort=p.quantity' . $url, true);
		$data['sort_status'] = $this->url->link('vendor/product', 'token=' . $this->session->data['token'] . '&sort=p.status' . $url, true);
		$data['sort_cstatus'] = $this->url->link('vendor/product', 'token=' . $this->session->data['token'] . '&sort=p.cstatus' . $url, true);
		$data['sort_order'] = $this->url->link('vendor/product', 'token=' . $this->session->data['token'] . '&sort=p.sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('vendor/product', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('vendor/product_list.tpl', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['product_id'])) {
			$data['action'] = $this->url->link('vendor/product/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('vendor/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('vendor/product', 'token=' . $this->session->data['token'] . $url, true);
		...
		} elseif (isset($product_info['vendor_id'])){
		...
		$this->response->setOutput($this->load->view('vendor/product_form.tpl', $data));
	}
