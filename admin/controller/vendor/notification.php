...
class ControllerVendorNotification extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('vendor/notification', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('vendor/notification', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('vendor/notification/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('vendor/notification/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			...
			$data['notifications'][] = array(
				...
				'date'			  => $result['date'],
				'edit'            => $this->url->link('vendor/notification/edit', 'token=' . $this->session->data['token'] . '&notification_id=' . $result['notification_id'] . $url, true)
			);
		...
		$data['sort_message'] = $this->url->link('vendor/notification', 'token=' . $this->session->data['token'] . '&sort=message' . $url, true);
		$data['sort_date'] = $this->url->link('vendor/notification', 'token=' . $this->session->data['token'] . '&sort=date' . $url, true);
		...
		$pagination->url = $this->url->link('vendor/notification', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('vendor/notification_list.tpl', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['notification_id'])) {
			$data['action'] = $this->url->link('vendor/notification/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('vendor/notification/edit', 'token=' . $this->session->data['token'] . '&notification_id=' . $this->request->get['notification_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('vendor/notification', 'token=' . $this->session->data['token'] . $url, true);
		...
		$this->response->setOutput($this->load->view('vendor/notification_form.tpl', $data));
	}
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['notification_message'] as $language_id => $value) {
			if ((utf8_strlen($value['message']) < 2) || (utf8_strlen($value['message']) > 255)) {
