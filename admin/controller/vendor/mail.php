...
class ControllerVendorMail extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('vendor/mail', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('vendor/mail', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('vendor/mail/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('vendor/mail/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			$data['mails'][] = array(
				...
				'edit'      => $this->url->link('vendor/mail/edit', 'token=' . $this->session->data['token'] . '&mail_id=' . $result['mail_id'] . $url, true)
			);
		}
		...
		$data['sort_name']     = $this->url->link('vendor/mail', 'token=' . $this->session->data['token'] . '&sort=name' . $url, true);
		$data['sort_date']     = $this->url->link('vendor/mail', 'token=' . $this->session->data['token'] . '&sort=date' . $url, true);
		...
		$pagination->url = $this->url->link('vendor/mail', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('vendor/mail_list.tpl', $data));
	}
	...
	protected function getForm() {
		...
		/* 18-06-2019 update replace text_sellerproduct */
		$data['sellertypes'][] = array(
	...
	//25-3-2019 start	
		...
		if (!isset($this->request->get['mail_id'])) {
			$data['action'] = $this->url->link('vendor/mail/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('vendor/mail/edit', 'token=' . $this->session->data['token'] . '&mail_id=' . $this->request->get['mail_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('vendor/mail', 'token=' . $this->session->data['token'] . $url, true);
		...
		$this->response->setOutput($this->load->view('vendor/mail_form.tpl', $data));
	}
