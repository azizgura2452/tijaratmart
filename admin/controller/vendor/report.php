...
class ControllerVendorReport  extends Controller {
	...
	public function delete() {
		...
		if (isset($this->request->get['filter_date'])) {
			...
			$this->response->redirect($this->url->link('vendor/report', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	public function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('vendor/report', 'token=' . $this->session->data['token'] . $url, true)
		);
	 	...
	 	if(isset($reports)) {
			...
			//		print_r($vendorstorename); die();
				...
				$data['reports'][] = array(
					...
					'view'          => $this->url->link('vendor/report/view', 'token=' . $this->session->data['token'] . '&order_id=' . $report['order_id'] . $url, true)
		...
		$data['column_prouctname']      = $this->language->get('column_prouctname');
		$data['column_status']          = $this->language->get('column_status');
		/* 2020 */		
		$data['column_storename']       = $this->language->get('column_storename');
		$data['column_adminstatus']     = $this->language->get('column_adminstatus');
		...
		$data['sort_order_id']  = $this->url->link('vendor/report', 'token=' . $this->session->data['token'] . '&sort=o.order_id' . $url, true);
		$data['sort_seller']    = $this->url->link('vendor/report', 'token=' . $this->session->data['token'] . '&sort=vop.seller' . $url, true);
		$data['sort_customer']  = $this->url->link('vendor/report', 'token=' . $this->session->data['token'] . '&sort=o.customer' . $url, true);
		$data['sort_product']   = $this->url->link('vendor/report', 'token=' . $this->session->data['token'] . '&sort=vop.product' . $url, true);
		$data['sort_status']  	= $this->url->link('vendor/report', 'token=' . $this->session->data['token'] . '&sort=o.status' . $url, true);
		
		$data['sort_total']   = $this->url->link('vendor/report', 'token=' . $this->session->data['token'] . '&sort=vop.total' . $url, true);
		
		$data['sort_date']  	= $this->url->link('vendor/report', 'token=' . $this->session->data['token'] . '&sort=vop.date' . $url, true);
		...
		$pagination->url   	= $this->url->link('vendor/report', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
	...
	public function view() {
		...
		/* 03 10 2019 replace order_id with order_product_id */
		...
		$data['invoice'] = $this->url->link('vendor/report/invoice', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], true);
		...
		foreach($products as $product){	
			...
			$option_data = array();
			$options = $this->model_vendor_vendor->getOrderOptions($order_id, $product['order_product_id']);
			...
			foreach ($options as $option) {
				...
				$data['chkshipcost'] = $this->config->get('shippingcost_status');
			...
			$data['products'][]=array(
				...
				'href'      => $this->url->link('catalog/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $product['product_id'] . $url, true),
				'sellerhref'=> $this->url->link('vendor/vendor/edit', 'token=' . $this->session->data['token'] . '&vendor_id=' . $ids . $url, true)
			);			
		}
		...
		$pagination->url = $this->url->link('vendor/report/view', 'token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'] . '&page={page}', true);
