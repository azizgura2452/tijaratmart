...
class ControllerVendorReviewField extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('vendor/review_field', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('vendor/review_field', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('vendor/review_field/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('vendor/review_field/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach($results as $result) {
			$data['review_fields'][] = array(
				...
				'status'	   => ($result['status'] ? $this->language->get('text_enable'): $this->language->get('text_disable')),
 				'edit'         => $this->url->link('vendor/review_field/edit', 'token=' . $this->session->data['token'] . '&rf_id=' . $result['rf_id'] . $url, true)
			);
		} 	
		...
		$data['sort_field_name'] = $this->url->link('vendor/review_field', 'token=' . $this->session->data['token'] . '&sort=fiels_name' . $url, true);
		$data['sort_sort_order'] = $this->url->link('vendor/review_field', 'token=' . $this->session->data['token'] . '&sort=sort_order' . $url, true);
		$data['sort_status'] = $this->url->link('vendor/review_field', 'token=' . $this->session->data['token'] . '&sort=status' . $url, true);
		...
		$pagination->url = $this->url->link('vendor/review_field', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('vendor/review_field_list.tpl', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['rf_id'])) {
			$data['action'] = $this->url->link('vendor/review_field/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('vendor/review_field/edit', 'token=' . $this->session->data['token'] . '&rf_id=' . $this->request->get['rf_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('vendor/review_field', 'token=' . $this->session->data['token'] . $url, true);
		...
		$this->response->setOutput($this->load->view('vendor/review_field_form.tpl', $data));
	}
