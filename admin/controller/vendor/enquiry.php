...
class ControllerVendorEnquiry extends Controller {
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			$this->response->redirect($this->url->link('vendor/enquiry', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('vendor/enquiry', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('vendor/enquiry/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('vendor/enquiry/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			...
			$data['inquires'][] = array(
				...
				'edit'        => $this->url->link('vendor/enquiry/edit', 'token=' . $this->session->data['token'] . '&inquiry_id=' . $result['inquiry_id'] . $url, true),
				'view'        => $this->url->link('vendor/enquiry/view', 'token=' . $this->session->data['token'] . '&inquiry_id=' . $result['inquiry_id'] . $url, true),
				// <!--07-03-2019 update-->
		...
		$data['sort_name']     = $this->url->link('vendor/enquiry', 'token=' . $this->session->data['token'] . '&sort=name' . $url, true);
		$data['sort_email']    = $this->url->link('vendor/enquiry', 'token=' . $this->session->data['token'] . '&sort=email' . $url, true);
		$data['sort_product']  = $this->url->link('vendor/enquiry', 'token=' . $this->session->data['token'] . '&sort=product' . $url, true);
		$data['sort_seller']   = $this->url->link('vendor/enquiry', 'token=' . $this->session->data['token'] . '&sort=seller' . $url, true);
		$data['sort_customer'] = $this->url->link('vendor/enquiry', 'token=' . $this->session->data['token'] . '&sort=customer' . $url, true);
		$data['sort_status']   = $this->url->link('vendor/enquiry', 'token=' . $this->session->data['token'] . '&sort=status' . $url, true);
		$data['sort_date']     = $this->url->link('vendor/enquiry', 'token=' . $this->session->data['token'] . '&sort=date' . $url, true);
		...
		$pagination->url = $this->url->link('vendor/enquiry', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		if(isset($data['filter_vendor'])){
			$sellers = $this->model_vendor_enquiry->getVendor($data['filter_vendor']);
		}
