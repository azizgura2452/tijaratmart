...
class ControllerVendorVendor extends Controller {
	...
	public function add() {	
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('vendor/vendor', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('vendor/vendor', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('vendor/vendor/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('vendor/vendor/delete', 'token=' . $this->session->data['token'] . $url, true);
        /* tmd vendor2 seler condtion  */
		$data['setting'] = $this->url->link('vendor/vendor/setting', 'token=' . $this->session->data['token'], true);
		/* tmd vendor2 seler condtion  */
		...
		foreach ($results as $result) {
			...
			if (!$result['approved']) {
				$approve = $this->url->link('vendor/vendor/approve', 'token=' . $this->session->data['token'] . '&vendor_id=' . $result['vendor_id'] . $url, true);
			} else {
				$approve = '';
			}
			
			if ($result['approved']) {
				$disapproved = $this->url->link('vendor/vendor/disapprove', 'token=' . $this->session->data['token'] . '&vendor_id=' . $result['vendor_id'] . $url, true);
			} else {
				$disapproved = '';
			}
			...
			$data['vendors'][] = array(
				...
				'image'		      => $image,
				'edit'            => $this->url->link('vendor/vendor/edit', 'token=' . $this->session->data['token'] . '&vendor_id=' . $result['vendor_id'] . $url, true)
			);
		}
		...
		$data['entry_vendorautoapprove']= $this->language->get('entry_vendorautoapprove');
		$data['entry_proautoapprove']= $this->language->get('entry_proautoapprove');
		$data['entry_customer2vendor']= $this->language->get('entry_customer2vendor');
		$data['entry_vendor2customer']= $this->language->get('entry_vendor2customer');
		$data['text_vendorlist']= $this->language->get('text_vendorlist');
		$data['text_settingvendor']= $this->language->get('text_settingvendor');
		$data['help_customer2vendor']= $this->language->get('help_customer2vendor');
		$data['help_vendor2customer']= $this->language->get('help_vendor2customer');
		$data['help_vendorautoapprove']= $this->language->get('help_vendorautoapprove');
		$data['help_proautoapprove']= $this->language->get('help_proautoapprove');
		$data['text_vendorsetingadd']= $this->language->get('text_vendorsetingadd');
        
        /* 28 02 2020  */
		 
		$data['entry_selername']= $this->language->get('entry_selername');
		$data['column_date']= $this->language->get('column_date');
		$data['column_storename']= $this->language->get('column_storename');
		$data['column_totalproduct']= $this->language->get('column_totalproduct');
		$data['column_approve']= $this->language->get('column_approve');
		...
		$data['sort_image'] 	= $this->url->link('vendor/vendor', 'token=' . $this->session->data['token'] . '&sort=image' . $url, true);
		$data['sort_display_name'] = $this->url->link('vendor/vendor', 'token=' . $this->session->data['token'] . '&sort=display_name' . $url, true);
		$data['sort_sellername'] = $this->url->link('vendor/vendor', 'token=' . $this->session->data['token'] . '&sort=vendorname' . $url, true);
		$data['sort_lastname'] 	= $this->url->link('vendor/vendor', 'token=' . $this->session->data['token'] . '&sort=lastname' . $url, true);
		$data['sort_email'] 	= $this->url->link('vendor/vendor', 'token=' . $this->session->data['token'] . '&sort=email' . $url, true);
		$data['sort_status'] 	= $this->url->link('vendor/vendor', 'token=' . $this->session->data['token'] . '&sort=status' . $url, true);
		$data['sort_sort_order']= $this->url->link('vendor/vendor', 'token=' . $this->session->data['token'] . '&sort=sort_order' . $url, true);
		/* 17 02 2020 */
		$data['sort_date_added'] = $this->url->link('vendor/vendor', 'token=' . $this->session->data['token'] . '&sort=date_added' . $url, true);
		$data['sort_totalproduct'] = $this->url->link('vendor/vendor', 'token=' . $this->session->data['token'] . '&sort=totalproduct' . $url, true);
		/* 17 02 2020 */
		...
		$pagination->url = $this->url->link('vendor/vendor', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['vendor_id'])) {
			$data['action'] = $this->url->link('vendor/vendor/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('vendor/vendor/edit', 'token=' . $this->session->data['token'] . '&vendor_id=' . $this->request->get['vendor_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('vendor/vendor', 'token=' . $this->session->data['token'] . $url, true);
...
//// Seller Store Start ///
		...
		} elseif (!empty($vendor_info['keyword'])){
	...
	protected function validateForm() {
		...
		if (utf8_strlen($this->request->post['keyword']) > 0) {
			$this->load->model('vendor/url_alias');

			$url_alias_info = $this->model_vendor_url_alias->getUrlAlias($this->request->post['keyword']);

			if ($url_alias_info && isset($this->request->get['vendor_id']) && $url_alias_info['query'] != 'vendor_id=' . $this->request->get['vendor_id']) {
				$this->error['keyword'] = sprintf($this->language->get('error_keyword'));
			}

			if ($url_alias_info && !isset($this->request->get['vendor_id'])) {
				$this->error['keyword'] = sprintf($this->language->get('error_keyword'));
			}
		}
	...
	public function setting() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate() ) {
			...
			$this->response->redirect($this->url->link('vendor/vendor/setting', 'token=' . $this->session->data['token'], true));
		}
		...
		$data['entry_hidevendorcontact']= $this->language->get('entry_hidevendorcontact');
		$data['help_hidevendorcontact']= $this->language->get('help_hidevendorcontact');		
       	$data['entry_vpostcode']= $this->language->get('entry_vpostcode');
		$data['help_postcode']= $this->language->get('help_postcode');      		
		$data['button_save'] = $this->language->get('button_save');
		$data['button_cancel'] = $this->language->get('button_cancel');
		/* 23 04 2020 */
		$data['text_selerlanguage'] = $this->language->get('text_selerlanguage');
		...
		$data['help_vendorterms'] = $this->language->get('help_vendorterms');
		/* 23 04 2020 */
		...
		$data['tab_selerlanguage'] = $this->language->get('tab_selerlanguage');
		$data['tab_vgeneral'] = $this->language->get('tab_vgeneral');
		$data['tab_vdata'] = $this->language->get('tab_vdata');
		$data['entry_vendorterms'] = $this->language->get('entry_vendorterms');
		$data['text_none'] = $this->language->get('text_none');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('vendor/vendor', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('vendor/vendor/setting', 'token=' . $this->session->data['token'], true);
		$data['cancel'] = $this->url->link('vendor/vendor', 'token=' . $this->session->data['token'], true);		
