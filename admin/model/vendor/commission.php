...
class ModelVendorCommission extends Model {
	...
	public function deleteCommission($commission_id) {
		$sql="delete  from " . DB_PREFIX . "vendor_commission where commission_id='".$commission_id."'";
		$query=$this->db->query($sql);
	}

	public function getCommission($commission_id) {
		$sql = "SELECT * FROM " . DB_PREFIX . "vendor_commission where commission_id='".$commission_id."'";
		$query = $this->db->query($sql);
		return $query->row;
	}
