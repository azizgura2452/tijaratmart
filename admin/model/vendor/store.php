...
class ModelVendorStore extends Model {
	public function addStore($data) {
		$sql="INSERT INTO " . DB_PREFIX . "vendor_store set vendor_id='".$data['vendor_id']."',country_id='".$data['country_id']."',zone_id='".$data['zone_id']."',logo='".$data['logo']."',store_about='".$this->db->escape($data['store_about'])."',banner='".$data['banner']."',email='".$this->db->escape($data['email'])."',address='".$this->db->escape($data['address'])."',phone='".$this->db->escape($data['phone'])."',city='".$this->db->escape($data['city'])."',postcode='".$this->db->escape($data['postcode'])."',bank_detail='".$this->db->escape($data['bank_detail'])."',tax_number='".$this->db->escape($data['tax_number'])."',shipping_charge='".$this->db->escape($data['shipping_charge'])."',commission='".$this->db->escape($data['commission'])."',date_added=now()";
		$this->db->query($sql);
	...
	public function editStore($vs_id, $data) {
		$sql="update " . DB_PREFIX . "vendor_store set vendor_id='".$data['vendor_id']."',country_id='".$data['country_id']."',zone_id='".$data['zone_id']."',store_about='".$this->db->escape($data['store_about'])."',logo='".$data['logo']."',banner='".$data['banner']."',email='".$this->db->escape($data['email'])."',address='".$this->db->escape($data['address'])."',phone='".$this->db->escape($data['phone'])."',city='".$this->db->escape($data['city'])."',postcode='".$this->db->escape($data['postcode'])."',bank_detail='".$this->db->escape($data['bank_detail'])."',tax_number='".$this->db->escape($data['tax_number'])."',shipping_charge='".$this->db->escape($data['shipping_charge'])."',commission='".$this->db->escape($data['commission'])."',date_modified=now() where vs_id='".$vs_id."'";
		$this->db->query($sql);
	...
	public function deleteStore($vs_id) {
		$sql="delete  from " . DB_PREFIX . "vendor_store where vs_id='".$vs_id."'";
		$query=$this->db->query($sql);
		$sql="delete  from " . DB_PREFIX . "vendor_store_description where vs_id='".$vs_id."'";
		$query=$this->db->query($sql);
	}

	public function getStore($vs_id) {
		$sql="select * from " . DB_PREFIX . "vendor_store where vs_id='".$vs_id."'";
		$query=$this->db->query($sql);
		return $query->row;
	}
	...
	public function getStores($data) {
		//$sql = "SELECT * FROM " . DB_PREFIX . "vendor_store where vs_id<>0";
