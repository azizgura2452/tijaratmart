...
class ModelVendorReviewField extends Model {
	...
	public function editReviewField($rf_id, $data) {
		$sql="update " . DB_PREFIX . "vendor_review_field set sort_order = '" . (int)$data['sort_order'] . "',status = '" . $data['status'] . "',date_modified=now() where rf_id='".$rf_id."'";
		$this->db->query($sql);
	...
	public function getTotalReviewFields($data) {
		//$sql ="SELECT COUNT(*) AS total FROM " . DB_PREFIX . "vendor_review_field where rf_id<>0";
