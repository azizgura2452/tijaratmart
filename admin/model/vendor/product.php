...
class ModelvendorProduct extends Model {
	...
	public function status($product_id){
		...
		if(isset($mailinfo['sellertype'])){
			...
			$replace = array(
				...
				'emails'	=> $emails,
				'loginlink' => $this->url->link('vendor/product', 'token=' . $this->session->data['token'] . '&product_id=' . $product_id . $url, true)
			);
	...
	public function getTotalProducts($data = array()) {
		$sql = "SELECT COUNT(DISTINCT p.product_id) AS total FROM " . DB_PREFIX . "vendor_to_product v2p LEFT JOIN " . DB_PREFIX . "product p ON (v2p.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (v2p.product_id = pd.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' and v2p.vendor_id<>0";
