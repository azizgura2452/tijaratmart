...
class ModelVendorReviewreport extends Model {
 	public function deleteReviewReport($review_id){		
		$sql="delete  from " . DB_PREFIX . "vendor_order_product where review_id='".$review_id."'";
	...
	public function getField($review_id,$vendor_id){
		$sql = "SELECT * FROM " . DB_PREFIX . "vendor_review_field_submit vrfs LEFT JOIN " . DB_PREFIX . "vendor_review_field_description vrfd ON (vrfs.rf_id = vrfd.rf_id) WHERE vrfs.vendor_id='".$vendor_id."' and vrfd.language_id ='". (int)$this->config->get('config_language_id')."' and vrfs.review_id='".$review_id."'";
		$query = $this->db->query($sql);
		return $query->rows;
	}

	public function getFieldById($review_id){
		$sql = "SELECT * FROM " . DB_PREFIX . "vendor_review_field_submit WHERE review_id='".$review_id."'";
		$query = $this->db->query($sql);
		return $query->row;
	}

	public function getReviewfeild($rf_id){
		$sql = "SELECT * FROM " . DB_PREFIX . "vendor_review_field_description WHERE rf_id='".$rf_id."' AND language_id ='". (int)$this->config->get('config_language_id')."'";
		$query = $this->db->query($sql);
		return $query->row;
	}
}
