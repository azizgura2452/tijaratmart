	...
	function getQuote($address) {
		...
		foreach ($products as $product) {
			...
			foreach($queryshippingcost->rows as $result){
				...
				$shippingcost[$product['product_id']]=$this->tax->calculate($result['price'], $this->config->get('shippingcost_tax_class_id'), $this->config->get('config_tax'));
			}
		...
		if ($status && $allprice!=0) {
			...
			$shipping_shippingcost = $this->config->get('shippingcost_shippingtitle');
			...
			$quote_data['shippingcost'] = array(
				...
				'title'        => $shippingcost,
				'cost'         => $this->tax->calculate($allprice, $this->config->get('shippingcost_tax_class_id'), $this->config->get('config_tax')),
				'tax_class_id' => 0,
				'text'         => $this->currency->format($this->tax->calculate($allprice, $this->config->get('shippingcost_tax_class_id'), $this->config->get('config_tax')), $this->session->data['currency'])
			);
			...
			$method_data = array(
				...
				'quote'      => $quote_data,
				'sort_order' => $this->config->get('shippingcost_sort_order'),
				'error'      => false
			);
		}
