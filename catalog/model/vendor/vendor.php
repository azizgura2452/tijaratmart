...
class ModelVendorVendor extends Model {
	public function addVendor($data) {
		...
		$sql = "INSERT INTO " . DB_PREFIX . "vendor SET display_name = '" . $this->db->escape($data['display_name']) . "',firstname = '" . $this->db->escape($data['firstname']) . "',lastname = '" . $this->db->escape($data['lastname']) . "',email = '" . $this->db->escape($data['email']) . "',telephone = '" . $this->db->escape($data['telephone']) . "',fax = '" . $this->db->escape($data['fax']) . "',company = '" . $this->db->escape($data['company']) . "',address_1 = '" . $this->db->escape($data['address_1']) . "',address_2 = '" . $this->db->escape($data['address_2']) . "',map_url = '" . $this->db->escape($data['map_url']) . "',  city = '" . $this->db->escape($data['city']) . "',country_id = '" .$data['country_id'] . "',zone_id = '" .$data['zone_id'] . "',salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . "',password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "',status = '1',approved = '".$autoapprovedseller."',about = '" . $this->db->escape($data['about']) . "',image='".$data['image']."',logo='".$data['logo']."',store_about='".$this->db->escape($data['store_about'])."',banner='".$data['banner']."',payment_method='".$this->db->escape($data['payment_method'])."',paypal = '" . $this->db->escape($data['paypal']) . "', bank_name = '" . $this->db->escape($data['bank_name']) . "', bank_branch_number = '" . $this->db->escape($data['bank_branch_number']) . "', bank_swift_code = '" . $this->db->escape($data['bank_swift_code']) . "', bank_account_name = '" . $this->db->escape($data['bank_account_name']) . "', bank_account_number = '" . $this->db->escape($data['bank_account_number']) . "',tax_number='".$this->db->escape($data['tax_number'])."',shipping_charge='".$this->db->escape($data['shipping_charge'])."', date_added = NOW(), postcode = '" .$this->db->escape($data['postcode']). "'";
		$this->db->query($sql);
		...
		if ($data['keyword']) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'vendor_id=" . (int)$vendor_id . "', keyword = '" . $this->db->escape($data['keyword']) . "'");
		}

	
		if (isset($data['chatstatus'])) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "vendor_chatsystem set message = '" . $this->db->escape($data['message']) . "',vendor_id='".$vendor_id."',chatstatus='".$data['chatstatus']."'");
		}
		
		
		/* Mail For Admin code Start */			
			$this->load->language('vendor/customer');
			
			$message  = $this->language->get('text_signup') . "<br/>";
			...
			$message .= $this->language->get('text_city') . ' ' . $data['city'] . "<br/>";
			...
			$mail->setSubject(html_entity_decode(sprintf($this->language->get('email_subject'), $this->request->post['firstname'].' '.$this->request->post['lastname']), ENT_QUOTES, 'UTF-8'));
	...
	public function editPassword($vendor_id,$data){
		$sql = "update " .DB_PREFIX . "vendor SET salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . "',password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "',date_modified=now() WHERE vendor_id = '" . $this->vendor->getId() ."'";
	...
	public function getVendorStoreDescriptions($vendor_id) {
		...
		foreach ($query->rows as $result) {
			...
			'name'=> $result['name'], 'meta_keyword'=> $result['meta_keyword'], 'description'=> $result['description'], 'meta_description'=> $result['meta_description'],  'shipping_policy'=> $result['shipping_policy'], 'return_policy'=> $result['return_policy'],
			);
	 	}
	...
	public function getVendor($vendor_id=0) {
		...
		$query = $this->db->query("SELECT DISTINCT *,v.vendor_id as vendor_id, (SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'vendor_id=" . (int)$vendor_id . "') AS keyword FROM " . DB_PREFIX . "vendor v LEFT JOIN " . DB_PREFIX . "vendor_description vd ON (v.vendor_id = vd.vendor_id) WHERE v.vendor_id = '" . (int)$vendor_id . "' AND vd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND approved!=0 AND vd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
	...
	public function getTotalOrders($vendor_id) {
		...
		if(isset($data['vendor_id'])){
			$sql .= " and vop.vendor_id='".$data['vendor_id']."'";
		}
		$query=$this->db->query($sql);
	...
	public function getOrders($data = array()){
		...
		$sql = "SELECT *, CONCAT(o.firstname, ' ', o.lastname) AS name, vop.order_status_id, vop.total as vtotal FROM " . DB_PREFIX . "vendor_order_product vop LEFT JOIN `" . DB_PREFIX . "order` o ON (vop.order_id = o.order_id) WHERE vop.order_status_id IN(" . implode(",", $implode) . ") ";
	...
	public function getTotalSellerReview($vendor_id) {		
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "vendor_review WHERE vendor_id='".$vendor_id."' AND review_id<>0 AND status=1";
	...
	public function addTracks($order_id, $data) {
		...
		date_modified=now() where order_id='".$order_id."' and vendor_id ='". $this->vendor->getId()."'";
		$this->db->query($sql);
	}
	...
	public function addOrdeStatus($order_id,$data,$vendor_id) {		
		...
		$sql = "SELECT * FROM " . DB_PREFIX . "vendor_order_product where order_id='".$order_id."' AND vendor_id ='". $this->vendor->getId()."'";
	...
	public function getTotalProducts($vendor_id){
		...
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "vendor_to_product v2p LEFT JOIN ".DB_PREFIX."product p ON(v2p.product_id = p.product_id) WHERE v2p.vendor_id='".$vendor_id."' AND p.status=1";
	...
	$sql = "INSERT INTO " . DB_PREFIX . "vendor_chat SET email = '" . $this->db->escape($data['email']) . "',subject = '" . $this->db->escape($data['subject']) . "',message = '" . $this->db->escape($data['message']) . "', attachimage = '" . $this->db->escape($data['attachimage']) . "', vendor_id = '" . (int)$vendor_id . "', customer_id='".$this->customer->getId()."', date_added = NOW()";
		$this->db->query($sql);
		$chat_id = $this->db->getLastId();
	 $this->db->query("INSERT INTO " . DB_PREFIX . "vendor_adminchat SET customer_id='".$this->customer->getId()."', vendor_id='".$vendor_id."', subject = '" . $this->db->escape($data['subject']) . "',message = '" . $this->db->escape($data['message']) . "', attachimage = '" . $this->db->escape($data['attachimage']) . "', chat_id = '" . (int)$chat_id . "',  user='customer', type='inbox', date_added = NOW()");	
	 $this->db->query("INSERT INTO " . DB_PREFIX . "vendor_customerchat SET customer_id='".$this->customer->getId()."', vendor_id='".$vendor_id."', subject = '" . $this->db->escape($data['subject']) . "',message = '" . $this->db->escape($data['message']) . "', attachimage = '" . $this->db->escape($data['attachimage']) . "', chat_id = '" . (int)$chat_id . "', user='customer', type='outbox', date_added = NOW()");	
	 $this->db->query("INSERT INTO " . DB_PREFIX . "vendor_vendorchat SET customer_id='".$this->customer->getId()."', vendor_id='".$vendor_id."', subject = '" . $this->db->escape($data['subject']) . "',message = '" . $this->db->escape($data['message']) . "', attachimage = '" . $this->db->escape($data['attachimage']) . "', chat_id = '" . (int)$chat_id . "', user='customer', type='inbox', date_added = NOW()");	
		...
		$querycust= $this->db->query("SELECT *  FROM " . DB_PREFIX . "customer WHERE customer_id = '" . $this->customer->getId() . "'");
	...
	public function getTotalOrderHistories($order_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "order_vendorhistory WHERE order_id = '" . (int)$order_id . "' AND vendor_id='".$this->vendor->getId()."'");
		return $query->row['total'];
	}
	...
	public function getSellerChat($vendor_id){
		$sql = "SELECT * FROM " . DB_PREFIX . "vendor_to_product WHERE vendor_id='".$vendor_id."'";
	...
	public function getTrackingCodeInfo($vendor_id, $order_id) {
		$sql = "SELECT * FROM " . DB_PREFIX . "vendor_order_product WHERE vendor_id='".$vendor_id."' AND order_id='".$order_id."' ";
		$query = $this->db->query($sql);
		return $query->row;
	}
	...
	public function getvendorOrdertotal($vendor_id, $order_id) { 
		...
		$sql="SELECT SUM(total) AS total FROM " . DB_PREFIX . "vendor_order_product WHERE vendor_id='".$vendor_id."' AND order_id='".$order_id."'";		
		$query = $this->db->query($sql);
	...
	public function getInProductSellerName($product_id, $vendor_id){
		$sql = "SELECT * FROM " . DB_PREFIX . "vendor_to_product vp LEFT JOIN  " . DB_PREFIX . "vendor v ON vp.vendor_id=v.vendor_id WHERE vp.product_id='".$product_id."' AND vp.vendor_id ='".$vendor_id."'";
