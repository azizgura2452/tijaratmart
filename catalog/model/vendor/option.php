<?php
class ModelVendorOption extends Model {
	public function addOption($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "vendor_to_option SET option_id = '" . (int)$option_id . "', vendor_id = '" . $this->vendor->getId() . "'");
	...
	public function getOptions($data = array()) {
		$sql = "SELECT * FROM `" . DB_PREFIX . "option` o LEFT JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id) LEFT JOIN " . DB_PREFIX . "vendor_to_option v2o ON (o.option_id = v2o.option_id) WHERE od.language_id = '" . (int)$this->config->get('config_language_id') . "' and v2o.vendor_id<>0";
		
		if(isset($data['vendor_id'])){
			$sql .= " and v2o.vendor_id='".$data['vendor_id']."'";
		}
	...
	public function getTotalOptions($data) {
		$sql = "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "option` o LEFT JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id) LEFT JOIN " . DB_PREFIX . "vendor_to_option v2o ON (o.option_id = v2o.option_id) WHERE od.language_id = '" . (int)$this->config->get('config_language_id') . "' and v2o.vendor_id<>0";
		if(isset($data['vendor_id'])){
			$sql .= " and v2o.vendor_id='".$data['vendor_id']."'";
		}
		$query = $this->db->query($sql);
