...
class ModelVendorEnquiry extends Model {
	...
	/// Seller And Customer To Mail ///
		...
		if(isset($product_info['product_id'])) {
			$vendor_info = $this->model_vendor_product->getProductVendor($product_info['product_id']);
		}
 		
		
		
		if(isset($vendor_info['vendor_id'])) {
			$seller_info = $this->model_vendor_product->getVendor($vendor_info['vendor_id']);
		} else {
			$seller_info = $this->model_vendor_product->getVendor($data['vendor_id']);
		}
