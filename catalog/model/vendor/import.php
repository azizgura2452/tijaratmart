...
class ModelVendorImport extends Model {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "vendor_to_manufacturer SET manufacturer_id = '" . (int)$manufacturer_id . "', vendor_id = '" . $vendor_id . "'");
	...
	public function option($data,$language_id,$vendor_id)
	{
		...
		$query=$this->db->query("select * from  " . DB_PREFIX . "option_description od  left join " . DB_PREFIX . "vendor_to_option vto on (vto.option_id=od.option_id) where  od.name = '" . $this->db->escape($optionname) . "' and vto.vendor_id='".$vendor_id."' limit 0,1");
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "vendor_to_option SET option_id = '" . (int)$option_id . "', vendor_id = '" . $vendor_id. "'");
	...
	{
		$query = $this->db->query("SELECT p.product_id FROM `" . DB_PREFIX . "product` p left join `" . DB_PREFIX . "vendor_to_product` vt on vt.product_id=p.product_id  where p.model='".$model."'");
	...
	{
		$query = $this->db->query("SELECT p.product_id FROM `" . DB_PREFIX . "product` p left join `" . DB_PREFIX . "vendor_to_product` vt on vt.product_id=p.product_id  where p.product_id='".$product_id."' AND vt.vendor_id = '" . $vendor_id . "'");
		if(isset($query->row['product_id']))
		{
			return $query->row['product_id'];
		}
	...
	public function addproduct($data,$language_id,$extra,$vendor_id)
	{
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "vendor_to_product SET product_id = '" . (int)$product_id . "', vendor_id = '" . $vendor_id . "'");
	...
	$this->db->query("DELETE FROM " . DB_PREFIX . "product_special WHERE product_id = '" . (int)$product_id . "'");
		...
		if (isset($data['attributes'])) {
            ...
            $this->db->query("DELETE FROM " . DB_PREFIX . "product_attribute WHERE product_id = '" . (int)$product_id . "' and attribute_id = '" . (int)$attribute['attribute_id'] . "' and language_id='".$language_id."'");
            $this->db->query("INSERT INTO " . DB_PREFIX . "product_attribute SET  	attribute_id = '" . (int)$attribute['attribute_id'] . "', product_id='".$product_id."', text = '" . $this->db->escape($attribute['text']). "',language_id = '".$language_id."'");
	...
	public function filtername($filtername,$language_id,$vendor_id)
	{
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "filter_description  fgd   left join " . DB_PREFIX . "vendor_to_option vto fgd.filter_id=fgd.filter_id where fgd.name='".$this->db->escape($data[0])."' and fgd.filter_group_id = '" . (int)$filter_group_id . "' and vto.vendor_id='".$vendor_id."'");
		if(!isset($query->row)) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "filter SET filter_group_id = '" . (int)$filter_group_id . "', sort_order = '" . (int)$data[1] . "'");
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "vendor_to_filter SET filter_id = '" . (int)$filter_id . "', vendor_id = '" . $vendor_id . "'");
		}	
		else
		{
	...
	$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "attribute_group_description agd  left join " . DB_PREFIX . "vendor_to_attribute_group vto on agd.attribute_group_id=agd.attribute_group_id where agd.name='".$this->db->escape($groupname)."' and agd.language_id = '" . (int)$language_id . "' and vto.vendor_id='".$vendor_id."'");
			if($query->row) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "vendor_to_attribute SET attribute_id = '" . (int)$attribute_id . "', vendor_id = '" . $vendor_id. "'");
