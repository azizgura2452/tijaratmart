<?php
class ModelVendorAttributeGroup extends Model {
	public function addAttributeGroup($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "vendor_to_attribute_group SET attribute_group_id = '" . (int)$attribute_group_id . "', vendor_id = '" . $this->vendor->getId() . "'");
	...
	public function getAttributeGroups($data = array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "attribute_group ag LEFT JOIN " . DB_PREFIX . "attribute_group_description agd ON (ag.attribute_group_id = agd.attribute_group_id) LEFT JOIN " . DB_PREFIX . "vendor_to_attribute_group v2ag ON (ag.attribute_group_id = v2ag.attribute_group_id) WHERE agd.language_id = '" . (int)$this->config->get('config_language_id') . "' and v2ag.vendor_id<>0";
		
		if(isset($data['vendor_id'])){
			$sql .= " and v2ag.vendor_id='".$data['vendor_id']."'";
		}
	...
	public function getTotalAttributeGroups($data) {		
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "attribute_group ag LEFT JOIN " . DB_PREFIX . "attribute_group_description agd ON (ag.attribute_group_id = agd.attribute_group_id) LEFT JOIN " . DB_PREFIX . "vendor_to_attribute_group v2ag ON (ag.attribute_group_id = v2ag.attribute_group_id) WHERE agd.language_id = '" . (int)$this->config->get('config_language_id') . "' and v2ag.vendor_id<>0";
