<?php
class ModelVendorCategory extends Model {
	...
	public function getCategories($data = array()) {
		...
		if (!empty($data['filter_name']) && empty($data['filter_category'])) {
			$sql .= " AND cd2.name LIKE '" . $this->db->escape($data['filter_name']) . "%'";
		}
		
		if(!empty($data['filter_category'])) {
		$sql .= " and  (c1.category_id = '" . (int)$data['filter_category'] . "' OR c1.parent_id = '" . (int)$data['filter_category'] . "')";
		}
		$sql .= " GROUP BY cp.category_id";
	...
	public function getTotalCategories($data) {
		...
		if(!empty($data['filter_category'])) {
		$sql .= " where c.category_id = '" . (int)$data['filter_category'] . "' OR c.parent_id = '" . (int)$data['filter_category'] . "'";
		}
		$query = $this->db->query($sql);
