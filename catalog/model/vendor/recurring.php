<?php
class ModelVendorRecurring extends Model {
	public function addRecurring($data) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "vendor_to_recurring WHERE recurring_id = '" . (int)$recurring_id . "'");
		$this->db->query("INSERT INTO " . DB_PREFIX . "vendor_to_recurring SET recurring_id = '" . (int)$recurring_id . "', vendor_id = '" . $this->vendor->getId() . "'");
	...
	public function getRecurrings($data = array()) {
		$sql = "SELECT * FROM `" . DB_PREFIX . "recurring` r LEFT JOIN " . DB_PREFIX . "recurring_description rd ON (r.recurring_id = rd.recurring_id) LEFT JOIN " . DB_PREFIX . "vendor_to_recurring v2r ON (r.recurring_id = v2r.recurring_id) WHERE rd.language_id = '" . (int)$this->config->get('config_language_id') . "' and v2r.vendor_id<>0";
		
		if(isset($data['vendor_id'])){
			$sql .= " and v2r.vendor_id='".$data['vendor_id']."'";
		}
	...
	public function getTotalRecurrings($data) {
		$sql = "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "recurring` r LEFT JOIN " . DB_PREFIX . "recurring_description rd ON (r.recurring_id = rd.recurring_id) LEFT JOIN " . DB_PREFIX . "vendor_to_recurring v2r ON (r.recurring_id = v2r.recurring_id) WHERE rd.language_id = '" . (int)$this->config->get('config_language_id') . "' and v2r.vendor_id<>0";
		if(isset($data['vendor_id'])){
			$sql .= " and v2r.vendor_id='".$data['vendor_id']."'";
		}
		$query = $this->db->query($sql);
