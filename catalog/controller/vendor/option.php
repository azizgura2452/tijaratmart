<?php
class ControllerVendorOption extends Controller {
	private $error = array();
	...
	public function index() {
		...
		$this->load->language('vendor/option');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_vendor_option->addOption($this->request->post);
			...
			$this->response->redirect($this->url->link('vendor/option'));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_vendor_option->editOption($this->request->get['option_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $option_id) {
				$this->model_vendor_option->deleteOption($option_id);
			}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('vendor/option')
		);

		$data['add'] = $this->url->link('vendor/option/add');
		$data['delete'] = $this->url->link('vendor/option/delete');
		...
		$option_total = $this->model_vendor_option->getTotalOptions($filter_data);

		$results = $this->model_vendor_option->getOptions($filter_data);
		...
		foreach ($results as $result) {
			$data['options'][] = array(
				...
				'edit'       => $this->url->link('vendor/option/edit','option_id=' . $result['option_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_name'] = $this->url->link('vendor/option','sort=od.name' . $url, 'SSL');
		$data['sort_sort_order'] = $this->url->link('vendor/option','sort=o.sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('vendor/option', $url . 'page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('vendor/option_list', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['option_id'])) {
			$data['action'] = $this->url->link('vendor/option/add');
		} else {
			$data['action'] = $this->url->link('vendor/option/edit','option_id=' . $this->request->get['option_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('vendor/option');

		if (isset($this->request->get['option_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$option_info = $this->model_vendor_option->getOption($this->request->get['option_id']);
		}
		...
		if (isset($this->request->post['option_description'])) {
			...
			$data['option_description'] = $this->model_vendor_option->getOptionDescriptions($this->request->get['option_id']);
		} else {
		...
		if (isset($this->request->post['option_value'])) {
			...
			$option_values = $this->model_vendor_option->getOptionValueDescriptions($this->request->get['option_id']);
		} else {
			$option_values = array();
		}
		...
		$this->response->setOutput($this->load->view('vendor/option_form', $data));
	}
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $option_id) {
			$product_total = $this->model_vendor_product->getTotalProductsByOptionId($option_id);
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$options = $this->model_vendor_option->getOptions($filter_data);

			foreach ($options as $option) {
				$option_value_data = array();

				if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'checkbox' || $option['type'] == 'image') {
					$option_values = $this->model_vendor_option->getOptionValues($option['option_id']);
