...
class ControllerVendorReview extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			if ((utf8_strlen($textss) > 120)) {
				$text = utf8_substr(trim(strip_tags(html_entity_decode($result['text'], ENT_QUOTES, 'UTF-8'))), 0, 119) .'<a class="readmore " data-toggle="modal" data-target="#viewfullreview'.$result['review_id'].'"> '.$this->language->get('text_readmore').'  </a>';	
			} else {
				$text = $result['text'];				
			}
		...
		$data['sort_product'] = $this->url->link('vendor/review','sort=pd.name' . $url, 'SSL');
		$data['sort_author'] = $this->url->link('vendor/review', 'sort=r.author' . $url, 'SSL');
		$data['sort_rating'] = $this->url->link('vendor/review','sort=r.rating' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('vendor/review','sort=r.status' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('vendor/review','sort=r.date_added' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('vendor/review',$url . 'page={page}', 'SSL');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['review_id'])) {
			...
			$data['action'] = $this->url->link('vendor/review/edit','&review_id=' . $this->request->get['review_id'] . $url, 'SSL');
		}
