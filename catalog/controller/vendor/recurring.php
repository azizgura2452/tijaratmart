<?php
class ControllervendorRecurring extends Controller {
	private $error = array();
	...
	public function index() {
		...
		$this->load->language('vendor/recurring');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_vendor_recurring->addRecurring($this->request->post);
			...
			$this->response->redirect($this->url->link('vendor/recurring'));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_vendor_recurring->editRecurring($this->request->get['recurring_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $recurring_id) {
				$this->model_vendor_recurring->deleteRecurring($recurring_id);
			}
	...
	public function copy() {
		...
		if (isset($this->request->post['selected']) && $this->validateCopy()) {
			foreach ($this->request->post['selected'] as $recurring_id) {
				$this->model_vendor_recurring->copyRecurring($recurring_id);
			}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('vendor/recurring')
		);

		$data['add'] = $this->url->link('vendor/recurring/add');
		$data['copy'] = $this->url->link('vendor/recurring/copy');
		$data['delete'] = $this->url->link('vendor/recurring/delete');
		...
		$recurring_total = $this->model_vendor_recurring->getTotalRecurrings($filter_data);

		$results = $this->model_vendor_recurring->getRecurrings($filter_data);
		...
		foreach ($results as $result) {
			$data['recurrings'][] = array(
				...
				'edit'         => $this->url->link('vendor/recurring/edit','recurring_id=' . $result['recurring_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_name'] = $this->url->link('vendor/recurring','sort=pd.name' . $url, 'SSL');
		$data['sort_sort_order'] = $this->url->link('vendor/recurring','sort=p.sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('vendor/recurring',$url . 'page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('vendor/recurring_list', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['recurring_id'])) {
			$data['action'] = $this->url->link('vendor/recurring/add');
		} else {
			$data['action'] = $this->url->link('vendor/recurring/edit','recurring_id=' . $this->request->get['recurring_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('vendor/recurring', '', 'SSL');

		if (isset($this->request->get['recurring_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$recurring_info = $this->model_vendor_recurring->getRecurring($this->request->get['recurring_id']);
		}
		...
		if (isset($this->request->post['recurring_description'])) {
			...
			$data['recurring_description'] = $this->model_vendor_recurring->getRecurringDescription($recurring_info['recurring_id']);
		} else {
		...
		$this->response->setOutput($this->load->view('vendor/recurring_form', $data));
	}
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $recurring_id) {
			$product_total = $this->model_vendor_product->getTotalProductsByProfileId($recurring_id);
