...
class ControllerVendorLatestOrder extends Controller {
	...
	public function letestview() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('vendor/order_report')
		);
		...
		foreach($products as $product){
			...
			$data['products'][]=array(
				...
				'href'      => $this->url->link('product/product','&product_id=' . $product['product_id'] . $url, 'SSL'),
				'sellerhref'=> $this->url->link('vendor/vendor_profile','&vendor_id=' . $ids . $url, 'SSL')
			);
		}
	...
	public function invoice() {
		...
		/* 03 10 2019  change all with order_id */
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
			/* 03 10 2019 remove function store and update anthor code */
