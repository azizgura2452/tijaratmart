		...
		if (!empty($setting['product'])) {
			...
			foreach ($products as $product_id) {
				...
				if ($product_info) {
					...
					if(!empty($ven_info)) {
					/* new code 10-1-2019 */
					if(isset($ven_info['vendor_id'])){
						$vendor_id = $ven_info['vendor_id'];
					} else {
						$vendor_id = '';
					}

					if(isset($ven_info['country_id'])){
						$country_id = $ven_info['country_id'];
					} else {
						$country_id = '';
					}

					if(isset($ven_info['zone_id'])){
						$zone_id = $ven_info['zone_id'];
					} else {
						$zone_id = '';
					}
					...
					if ($ven_info['image']) {
						$sellerimage = $this->model_tool_image->resize($ven_info['image'], 60, 60);
					} else {
						$sellerimage = $this->model_tool_image->resize('placeholder.png', 60, 60);
					}

					$store_info = $this->model_vendor_allseller->getVendordescription($vendor_id);
					...
					$data['products'][] = array(
						...
						'profile'     => $this->url->link('vendor/vendor_profile', 'vendor_id=' . $vendor_id)
					);
					}
